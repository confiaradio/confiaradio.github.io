...
$audioLineNumber = Select-String -Path index.html -Pattern "http://stream.zeno.fm/h4cwrur8uwzuv" | Select-Object LineNumber -ExpandProperty LineNumber

if($audioLineNumber-1 -eq -1 ){
    # Audio player script
    $textToAdd = '<div class="mb-4"><audio controls autoplay><source src="http://stream.zeno.fm/h4cwrur8uwzuv" type="audio/ogg">Your browser does not support the audio element.</audio></div>'
